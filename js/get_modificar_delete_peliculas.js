...
document.addEventListener('DOMContentLoaded', async () => {
   ...
   // const response = await fetch('http://localhost:8080/apisimple/peliculas', options);
    const response = await fetch('https://desarrollamos.org/phpmovies-api/peliculas', options);
    ...
    movies.forEach(movie => {
        ...
        idMovie.textContent = movie.id_pelicula;
    ...
    document.querySelectorAll('.btnModificar').forEach(button => {
        button.addEventListener('click', async (event) => {
            ...
            try {
                const response = await fetch(`https://desarrollamos.org/phpmovies-api/peliculas?id=${peliculaId}`);
                if (!response.ok) {
